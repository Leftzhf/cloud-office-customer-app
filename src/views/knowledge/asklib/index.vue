<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="留言内容" prop="content" min-width="160px" align="center"/>
      <el-table-column label="电子邮箱" prop="email" min-width="160px" align="center"/>
      <el-table-column label="电话" prop="phoneNumber" min-width="160px" align="center"/>
    ...
    <el-dialog :title="dialogTitleMap[dialogStatus]" :visible.sync="dialogFormVisible" width="880px">
      <el-form :model="editForm" label-width="80px">
        ...
        <el-form-item label="电话">
          <el-input v-model="editForm.phoneNumber" :disabled="true"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      leaveInfoApi.getLeaveInfoList(this.listQuery).then((response) => {
    ...
    deleteData(faqId) {
      leaveInfoApi.deleteLeaveInfo(faqId).then((response) => {
    ...
    updateData() {
      ...
      leaveInfoApi.updateLeaveInfo(data).then((response) => {
        if (response.status === 200) {
          // 隐藏弹窗并弹窗通知提示
          this.dialogFormVisible = false
          this.$notify({
